    ...
    <div style={{ backgroundColor: "var(--gc-surface)", border: "1px solid var(--gc-border)", borderRadius: "0px", padding: "var(--gc-space-4)" }}>
      ...
      <ResponsiveContainer width="100%" height={Math.max(200, data.length * 40)}>
        <BarChart data={data} layout="vertical" margin={{ left: 80, right: 16, top: 0, bottom: 0 }}>
          ...
          <Tooltip contentStyle={{ backgroundColor: "var(--gc-surface)", border: "1px solid var(--gc-border)", borderRadius: "0px", fontFamily: "var(--gc-font-body)", fontSize: 12 }} formatter={(v: number) => [valueFormatter(v), "Value"]} />
