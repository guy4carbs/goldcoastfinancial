...
function getActiveGcTheme(): string {
...
const BASE_CONFIG: Partial<DriverConfig> = {
  ...
  // driver.js appends popover to <body>, outside GCShell's themed subtree.
  // Copy the active data-theme so gc-tokens CSS vars cascade to the popover.
  onPopoverRender: (popover) => {
    popover.wrapper.setAttribute("data-theme", getActiveGcTheme());
  },
      ...
      return steps.map((step, idx, arr) => {
        ...
        if (isLast && hasChain) {
          ...
          onNextClick = (el, step, ctx) => {
            ...
            // /hcms/agents/:id when nextConfig.page is a dynamic prefix).
