...
const STORAGE_KEY = "heritage.tour.state.v1";

const defaultState: TourPersistedState = {
  completedTourIds: [],
  resume: null,
  firstLoginDismissed: { agent: false, crm: false },
  lastSeenAt: 0,
};

function safeParse(raw: string | null): TourPersistedState {
  if (!raw) return { ...defaultState, firstLoginDismissed: { agent: false, crm: false } };
  try {
    ...
    return {
      ...
      resume:
        parsed.resume && typeof parsed.resume.tourId === "string"
          ...
          : null,
      // Migration:
      //  - Legacy boolean → treat as both roles dismissed (don't re-nag existing users).
      //  - Object → preserve known role flags, default missing keys to false so existing
      //    users see new lounge tours on their first visit.
      firstLoginDismissed: (() => {
        ...
        return { agent: legacy, crm: legacy };
      })(),
      lastSeenAt: typeof parsed.lastSeenAt === "number" ? parsed.lastSeenAt : 0,
    };
  } catch {
    return { ...defaultState, firstLoginDismissed: { agent: false, crm: false } };
  }
}
