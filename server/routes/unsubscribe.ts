...
import { z } from "zod";
import { pool } from "../db";
// Parallel workstream — server/services/email/ (suppression + token).
// These resolve once that module lands; the contract below is canonical.
 ...
 *
 * Atlas mounts this router at /api/unsubscribe.
 * ALL endpoints are PUBLIC — no auth, no session, no CSRF.
 * The HMAC unsubscribe token is the only gate.
 *
 * Mirrors the conventions of the legacy newsletter unsubscribe
 * (routes.ts:3975-4046): email masking, { success } response shapes,
 * subscriber_activity logging, idempotent behavior.
 */
 ...
 *  - flips newsletter_subscribers to 'unsubscribed' if a row matches
 *  - logs to subscriber_activity (matching legacy newsletter pattern)
 */
async function applyUnsubscribe(
  ...
  // 3) Newsletter list — flip to unsubscribed + log activity (legacy parity).
  const subResult = await pool.query(
    `SELECT id, status FROM newsletter_subscribers WHERE lower(email) = lower($1)`,
    [email]
  );
  if (subResult.rows.length > 0) {
    ...
    await pool.query(
      `INSERT INTO subscriber_activity (subscriber_id, activity_type, description, performed_by)
       VALUES ($1, 'unsubscribed', 'User unsubscribed via unsubscribe link', 'user')`,
      [subscriber.id]
    );
  }
}
 ...
 * Suppresses the email + stops enrollments + newsletter. Idempotent.
 */
 ...
 *
 * express.urlencoded() is registered globally at server/index.ts:215,
