...
// Original blog posts from Blog.tsx - exact copy
const blogPosts = [
  // ==================== TERM LIFE ====================
  {
    id: 1,
...
async function restoreBlogPosts() {
  console.log("Starting blog post restoration...");
  ...
  try {
    ...
    console.log("Inserting original blog posts...");

    for (const post of blogPosts) {
      const publishedAt = new Date();
      // Set date based on the post's date string
      ...
      if (dateMatch) {
        ...
        publishedAt.setFullYear(parseInt(dateMatch[3]));
        publishedAt.setMonth(months[dateMatch[1]]);
        publishedAt.setDate(parseInt(dateMatch[2]));
      }

      await pool.query(
        `INSERT INTO blog_posts (
          slug, title, excerpt, content, category, author,
          featured_image, read_time_minutes, is_featured, status,
    ...
    console.log(`\nSuccessfully restored ${blogPosts.length} blog posts!`);

    // Verify
    const result = await pool.query("SELECT COUNT(*) FROM blog_posts");
    console.log(`Total blog posts in database: ${result.rows[0].count}`);
